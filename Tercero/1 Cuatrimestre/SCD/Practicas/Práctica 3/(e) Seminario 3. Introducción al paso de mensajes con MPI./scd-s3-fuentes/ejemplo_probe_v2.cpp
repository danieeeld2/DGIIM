...
//  V2 : las cadenas se construyen usando 'string' cuando incluyen números, no con snprintf
//
// Historial:
// Actualizado a C++11 en Septiembre de 2017
// Se ha creado esta versión v2 en Diciembre de 2020
// -----------------------------------------------------------------------------
...
void funcion_emisores( int id_propio )
{
   ...
   str = "inicio del proceso número: " + to_string( id_propio );
   enviar_cadena( str.c_str() );

   for ( int i = 0 ; i < num_iteraciones_por_emisor ; i++ )
   {
      int dur_ms = aleatorio<20,200>();
      using namespace std ;
      str = "proceso: " + to_string( id_propio ) + " voy a dormir " + to_string( dur_ms ) + " milisegundos" ;
      enviar_cadena( str.c_str() );
      this_thread::sleep_for( chrono::milliseconds( dur_ms ) );
      enviar_cadena("ya he dormido");
   }
   str = "fin del proceso número " + to_string(id_propio) ;
   enviar_cadena( str.c_str() );
}
