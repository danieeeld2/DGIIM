...
void funcion_filosofos(int id)
{
   ...
   while (true)
   {
      ...
      if(propina == 5)
         etiq_auxiliar_propina = etiq_propina_maxima;
      else
         etiq_auxiliar_propina = etiq_propina;
      MPI_Ssend(&valor, 1, MPI_INT, id_valido, etiq_levantarse, MPI_COMM_WORLD);
      cout << "Filosofo " << id << " se levanta de la mesa " << mesa << endl;
      MPI_Send(&propina,1,MPI_INT,id_valido,etiq_auxiliar_propina,MPI_COMM_WORLD);
...
void funcion_camarero(int id)
{
   ...
   while (true)
   {
      // Da prioridad a la propina máxima
      MPI_Iprobe(MPI_ANY_SOURCE,etiq_propina_maxima,MPI_COMM_WORLD,&flag,&estado);
      if(flag){ // Si hay propina maxima da prioridad
         MPI_Recv(&valor,1,MPI_INT,MPI_ANY_SOURCE,etiq_propina_maxima,MPI_COMM_WORLD,&estado);
         num_propinas_Recibidas++;
         suma_propina += valor;
         cout << "El camarero " << id << " recibe una propina máxima del filósofo " << estado.MPI_SOURCE
              << ".Ha recibido en total " << num_propinas_Recibidas << " propinas" << endl;
      }else{
         // Si no hay propina máxima miramos si hay otra propina
         MPI_Iprobe(MPI_ANY_SOURCE,etiq_propina,MPI_COMM_WORLD,&flag,&estado);
         if(flag){   // Si no hay propina máxima, comprueba que hay una menor
            MPI_Recv(&valor,1,MPI_INT,MPI_ANY_SOURCE,etiq_propina,MPI_COMM_WORLD,&estado);
            num_propinas_Recibidas++;
            suma_propina += valor;
      ...
      if (libres == 0)  // Si están todos sentados, solo pueden levantarse
         MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_levantarse, MPI_COMM_WORLD, &estado);
      else{ // En otro caso pueden sentarse y levantarse
         MPI_Iprobe(MPI_ANY_SOURCE,etiq_levantarse,MPI_COMM_WORLD,&flag,&estado);
         if(flag){   // Miramos primero si alguno quiere sentarse
            MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_levantarse, MPI_COMM_WORLD, &estado);
         }else{   // Miramos si alguno quiere sentarse
            MPI_Iprobe(MPI_ANY_SOURCE,etiq_sentarse,MPI_COMM_WORLD,&flag,&estado);
            if(flag)
